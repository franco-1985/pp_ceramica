...
from src.crud.cliente import get_clientes, get_cliente_by_id
...
@cliente.get("/cliente/{id}", response_model=list[ClienteRequest], tags=["Cliente"], name="Devolver un cliente")
def get_by_id(id: int, db: Session = Depends(get_db)):
    try:
        return get_cliente_by_id(db=db, id_cliente=id)
    except Exception as ex:
        msg = {'status': -1,
               "mensaje:": str(ex.detail)}
        status = ex.status_code
